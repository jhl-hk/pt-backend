...
- `buildPath()` — the core path-filtering logic: deduplicates consecutive ASN prepending, collapses unknown transit ASNs to `[0, lastKnown]`, detects loops (stops if RootASN appears twice), and drops RootASN when followed by a Tier-1 ASN
